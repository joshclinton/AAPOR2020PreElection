##############################################################################
##  Analyze Siena/NYT Michigan Data
...
# NOTE: MI ALLOWS SAME DAY REGISTRATION
...
dat <- read_sav("Google Drive/AAPOR.PreElectionPolling/Other Data/2020 General State Level Polls/Siena_Final Datasets/datasetMI.sav")
...
Siena_MI <- merge(dat,mi_ctyinfo,by="county_name")
...
Siena_MI$EDVote <- 0
Siena_MI$EDVote[Siena_MI$QMETH2==1] <- 1
Siena_MI$AbsenteeVote <- 0
Siena_MI$AbsenteeVote[Siena_MI$QEVMETH==1] <- 1
Siena_MI$AbsenteeVote[Siena_MI$QEVMETH==2] <- 1
Siena_MI$AbsenteeVote[Siena_MI$QMETH2==2] <- 1
Siena_MI$AbsenteeVote[Siena_MI$QMETH2==3] <- 1

table(Siena_MI$EDVote,Siena_MI$AbsenteeVote)  # 34 have neither!

for(i in 1:5){
  datsub <- subset(Siena_MI,party_stratum==i)
...
for(i in 1:5){
  Siena_MI$ModeWgt[Siena_MI$party_stratum==i & Siena_MI$EDVote==1] <- EDWgt[i]
  Siena_MI$ModeWgt[Siena_MI$party_stratum==i & Siena_MI$AbsenteeVote==1] <- AbsWgt[i]
}

# How correlated are these?  
cor(Siena_MI$final_weight,Siena_MI$ModeWgt,use="pairwise.complete")
...
Siena_MI$WeightModeWgt <- Siena_MI$ModeWgt*Siena_MI$final_weight
...
for(i in 1:5){
  Siena_MI$VoteWgt[Siena_MI$party_stratum==i & Siena_MI$QPRES==1] <- BidenWgt[i]
  Siena_MI$VoteWgt[Siena_MI$party_stratum==i & Siena_MI$QPRES==2] <- TrumpWgt[i]
}

# How correlated are these?  
cor(Siena_MI$final_weight,Siena_MI$VoteWgt,use="pairwise.complete")

Siena_MI$WeightVoteModeWgt <- Siena_MI$VoteWgt*Siena_MI$WeightModeWgt
...
TrumpStrata.MI <- Poll3.Trump.Vote.Strata - Poll.Trump.Vote.Strata
BidenStrata.MI <- Poll3.Biden.Vote.Strata - Poll.Biden.Vote.Strata
...
Siena_MI$QRACE[Siena_MI$QRACE==4] <- 5
Siena_MI$QRACE[Siena_MI$QRACE==6] <- 5
Siena_MI$QRACE[Siena_MI$QRACE==9] <- NA
Siena_MI$GENDER[Siena_MI$GENDER==3] <- NA

Siena_MI$QVOTE16[Siena_MI$QVOTE16==5] <- 8
Siena_MI$QVOTE16[Siena_MI$QVOTE16==7] <- 6

png(file="Google Drive/AAPOR.PreElectionPolling/Writing/Figures/MIAdjust_Siena.png",width=600, height=900)
...
datsub <- na.omit(as.data.frame(cbind(Siena_MI$QRACE,Siena_MI$final_weight,Siena_MI$WeightVoteModeWgt)))
...
# Education
datsub <- na.omit(as.data.frame(cbind(Siena_MI$QEDUC4,Siena_MI$final_weight,Siena_MI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_MI$QEDUC4)
...
# Gender
datsub <- na.omit(as.data.frame(cbind(Siena_MI$GENDER,Siena_MI$final_weight,Siena_MI$WeightVoteModeWgt)))
...
# Age
datsub <- na.omit(as.data.frame(cbind(Siena_MI$AGE4,Siena_MI$final_weight,Siena_MI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_MI$AGE4)[1:4]
...
datsub <- na.omit(as.data.frame(cbind(Siena_MI$QVOTE16,Siena_MI$final_weight,Siena_MI$WeightVoteModeWgt)))
...
# PARTY
datsub <- na.omit(as.data.frame(cbind(Siena_MI$QPARTYID,Siena_MI$final_weight,Siena_MI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_MI$QPARTYID)
...
ChangeMI <- table(Vote2106 = Siena_MI$QVOTE16, Vote2020 = Siena_MI$QPRES)

# New Voters
sum(as.logical(Siena_MI$QVOTE16==8)*Siena_MI$final_weight)/sum(Siena_MI$final_weight)
sum(as.logical(Siena_MI$QVOTE16==8 & Siena_MI$QPRES==1)*Siena_MI$final_weight)/sum(as.logical(Siena_MI$QVOTE16==8)*Siena_MI$final_weight)
sum(as.logical(Siena_MI$QVOTE16==8 & Siena_MI$QPRES==2)*Siena_MI$final_weight)/sum(as.logical(Siena_MI$QVOTE16==8)*Siena_MI$final_weight)
sum(as.logical(Siena_MI$QVOTE16==8 & Siena_MI$QPRES==9)*Siena_MI$final_weight)/sum(as.logical(Siena_MI$QVOTE16==8)*Siena_MI$final_weight)
