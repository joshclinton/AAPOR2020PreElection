...
dat <- read_sav("Google Drive/AAPOR.PreElectionPolling/Other Data/2020 General State Level Polls/Siena_Final Datasets/datasetTX.sav")
dat$county_name <- tolower(dat$County)

# Get voter file data

TX_Mode <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/TX/Strata/TX_TargetSmart_PartyStrataVote_2020.csv")
TX_Vote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/TX/Strata/TX_NEAT_PartyStrataVote_2020.csv")

# Get NEAT data for county stratification
tx_ctyvote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/TX/County/TX_NEAT_CountyVote_2020.csv")
tx_ctyvote$county_name <- tolower(tx_ctyvote$county_name)

# Extract information
tx_ctyinfo <- tx_ctyvote[,c("fips_code","county_name","party_stratum","party_stratum_name","geo_stratum","geo_stratum_name")]

Trump.Vote.Strata <- TX_Vote[,c("party_stratum","Trump")]
Biden.Vote.Strata <- TX_Vote[,c("party_stratum","Biden")]
Absentee.Vote.Strata <- TX_Mode[,c("party_stratum","Absentee")]
ElectionDay.Vote.Strata <- TX_Mode[,c("party_stratum","Election.Day")] 
Early.Vote.Strata <- TX_Mode[,c("party_stratum","Early")]
TotalVote <- sum(TX_Vote$Trump+TX_Vote$Biden)
TotalVote2 <- sum(TX_Mode$Absentee+TX_Mode$Election.Day + TX_Mode$Early)
...
Siena_TX <- merge(dat,wi_ctyinfo,by="county_name")
...
Siena_TX$EDVote <- 0
Siena_TX$EDVote[Siena_TX$QMETH2==1] <- 1
Siena_TX$EarlyVote <- 0
Siena_TX$EarlyVote[Siena_TX$QEVMETH==1] <- 1
Siena_TX$EarlyVote[Siena_TX$QMETH2==2] <- 1
Siena_TX$AbsenteeVote <- 0
Siena_TX$AbsenteeVote[Siena_TX$QMETH2==3] <- 1
Siena_TX$AbsenteeVote[Siena_TX$QEVMETH==2] <- 1

for(i in 1:5){
  datsub <- subset(Siena_TX,party_stratum==i)
...
for(i in 1:5){
  Siena_TX$ModeWgt[Siena_TX$party_stratum==i & Siena_TX$EDVote==1] <- EDWgt[i]
  Siena_TX$ModeWgt[Siena_TX$party_stratum==i & Siena_TX$AbsenteeVote==1] <- AbsWgt[i]
  Siena_TX$ModeWgt[Siena_TX$party_stratum==i & Siena_TX$EarlyVote==1] <- EarlyWgt[i]
}

# How correlated are these?  
cor(Siena_TX$final_weight,Siena_TX$ModeWgt,use="pairwise.complete")
...
Siena_TX$WeightModeWgt <- Siena_TX$ModeWgt*Siena_TX$final_weight
...
for(i in 1:5){
  Siena_TX$VoteWgt[Siena_TX$party_stratum==i & Siena_TX$QPRES==1] <- BidenWgt[i]
  Siena_TX$VoteWgt[Siena_TX$party_stratum==i & Siena_TX$QPRES==2] <- TrumpWgt[i]
}

# How correlated are these?  
cor(Siena_TX$final_weight,Siena_TX$VoteWgt,use="pairwise.complete")

Siena_TX$WeightVoteModeWgt <- Siena_TX$VoteWgt*Siena_TX$WeightModeWgt
...
Poll3.Trump.Vote.Strata - Poll.Trump.Vote.Strata
Poll3.Biden.Vote.Strata - Poll.Biden.Vote.Strata
...
Siena_TX$QRACE[Siena_TX$QRACE==4] <- 5
Siena_TX$QRACE[Siena_TX$QRACE==6] <- 5
Siena_TX$QRACE[Siena_TX$QRACE==9] <- NA
Siena_TX$GENDER[Siena_TX$GENDER==3] <- NA

Siena_TX$QVOTE16[Siena_TX$QVOTE16==5] <- 8
Siena_TX$QVOTE16[Siena_TX$QVOTE16==7] <- 6

pdf(file="Google Drive/AAPOR.PreElectionPolling/Writing/Figures/TXAdjust_Siena.pdf")
...
plot(seq(1,ncompare),seq(1,ncompare),xlim=c(a,b),type="n",axes=FALSE,ylab="",
     ...
     main="TX: Effect of Reweighting by Vote and Mode by County Strata")
axis(1)
...
datsub <- na.omit(as.data.frame(cbind(Siena_TX$QRACE,Siena_TX$final_weight,Siena_TX$WeightVoteModeWgt)))
...
# Education
datsub <- na.omit(as.data.frame(cbind(Siena_TX$QEDUC4,Siena_TX$final_weight,Siena_TX$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_TX$QEDUC4)
points(as.numeric(tb),seq(4,8),pch=18)
text(rep(a,times=length(labt)),seq(4,8),labt,pos=4)
abline(h=8.5)

# Gender
datsub <- na.omit(as.data.frame(cbind(Siena_TX$GENDER,Siena_TX$final_weight,Siena_TX$WeightVoteModeWgt)))
...
# Age
datsub <- na.omit(as.data.frame(cbind(Siena_TX$AGE4,Siena_TX$final_weight,Siena_TX$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_TX$AGE4)[1:4]
...
datsub <- na.omit(as.data.frame(cbind(Siena_TX$QVOTE16,Siena_TX$final_weight,Siena_TX$WeightVoteModeWgt)))
...
# PARTY
datsub <- na.omit(as.data.frame(cbind(Siena_TX$QPARTYID,Siena_TX$final_weight,Siena_TX$WeightVoteModeWgt)))
...
labt <- get_labels(Siena_TX$QPARTYID)
...
table(Vote2106=Siena_TX$QVOTE16,Vote2020=Siena_TX$QPRES)
