...
CNN_NC <- read_spss("Google Drive/AAPOR.PreElectionPolling/Other Data/2020 General State Level Polls/CNN_Final Datasets/CNN_NC.sav")
CNN_NC$fips_code <- CNN_NC$merge_fips
CNN_NC$weight <- CNN_NC$WEIGHT
# Get voter file data
...
CNN_NC <- merge(CNN_NC,nc_ctyinfo,by="fips_code")
#CNN_NC$party_stratum <- CNN_NC$party_stratum.x
nobs2 <- nrow(CNN_NC)
...
CNN_NC$EDVote <- ifelse(CNN_NC$votemode==1,1,0)
CNN_NC$EarlyVote <- ifelse(CNN_NC$votemode==3,1,0)
CNN_NC$Absentee <- ifelse(CNN_NC$votemode==2,1,0)

for(i in 1:5){
  datsub <- subset(CNN_NC,party_stratum==i)
...
for(i in 1:5){
  CNN_NC$ModeWgt[CNN_NC$party_stratum==i & CNN_NC$EDVote==1] <- EDWgt[i]
  CNN_NC$ModeWgt[CNN_NC$party_stratum==i & CNN_NC$EarlyVote==1] <- EarlyWgt[i]
  CNN_NC$ModeWgt[CNN_NC$party_stratum==i & CNN_NC$Absentee==1] <- AbsWgt[i]
}

# How correlated are these?  
cor(CNN_NC$weight,CNN_NC$ModeWgt,use="pairwise.complete")
...
CNN_NC$WeightModeWgt <- CNN_NC$ModeWgt*CNN_NC$weight
...
for(i in 1:5){
  CNN_NC$VoteWgt[CNN_NC$party_stratum==i & CNN_NC$presvote==1] <- BidenWgt[i]
  CNN_NC$VoteWgt[CNN_NC$party_stratum==i & CNN_NC$presvote==2] <- TrumpWgt[i]
}

# How correlated are these?  
cor(CNN_NC$weight,CNN_NC$VoteWgt,use="pairwise.complete")

CNN_NC$WeightVoteModeWgt <- CNN_NC$VoteWgt*CNN_NC$WeightModeWgt
...
png(file="Google Drive/AAPOR.PreElectionPolling/Writing/Figures/CNNNCAdjust.png",width=600, height=900)
a <- -.1
b <- .05
ncompare <- 28
...
axis(1)
abline(v=seq(-.05,.05,by=.01),lty=2,col="GREY")
abline(v=0)

# White/Non-White
datsub <- na.omit(as.data.frame(cbind(CNN_NC$white,CNN_NC$weight,CNN_NC$WeightVoteModeWgt)))
...
labt <- get_labels(CNN_NC$white)
points(as.numeric(tb),seq(1,3),pch=18)
text(rep(a,times=length(labt)),seq(1,3),labt,pos=4)
abline(h=3.5)

# Education
datsub <- na.omit(as.data.frame(cbind(CNN_NC$educ,CNN_NC$weight,CNN_NC$WeightVoteModeWgt)))
...
labt <- get_labels(CNN_NC$educ)
points(as.numeric(tb),seq(4,8),pch=18)
text(rep(a,times=length(labt)),seq(4,8),labt,pos=4)
abline(h=8.5)

# income
datsub <- na.omit(as.data.frame(cbind(CNN_NC$income3,CNN_NC$weight,CNN_NC$WeightVoteModeWgt)))
...
labt <- get_labels(CNN_NC$income3)
points(as.numeric(tb),seq(9,12),pch=18)
text(rep(a,times=length(labt)),seq(9,12),labt,pos=4)
abline(h=12.5)

# Gender
datsub <- na.omit(as.data.frame(cbind(CNN_NC$sex,CNN_NC$weight,CNN_NC$WeightVoteModeWgt)))
...
labt <- get_labels(CNN_NC$sex)
points(as.numeric(tb),seq(13,14),pch=18)
text(rep(a,times=length(labt)),seq(13,14),labt,pos=4)
abline(h=14.5)

# Age
datsub <- na.omit(as.data.frame(cbind(CNN_NC$age,CNN_NC$weight,CNN_NC$WeightVoteModeWgt)))
...
labt <- get_labels(CNN_NC$age)
points(as.numeric(tb),seq(15,19),pch=18)
text(rep(a,times=length(labt)),seq(15,19),labt,pos=4)
abline(h=19.5)

# ideology
datsub <- na.omit(as.data.frame(cbind(CNN_NC$ideology,CNN_NC$weight,CNN_NC$WeightVoteModeWgt)))
...
labt <- get_labels(CNN_NC$ideology)
points(as.numeric(tb),seq(20,23),pch=18)
text(rep(a,times=length(labt)),seq(20,23),labt,pos=4)
abline(h=23.5)

# PARTY5
datsub <- na.omit(as.data.frame(cbind(CNN_NC$party,CNN_NC$weight,CNN_NC$WeightVoteModeWgt)))
