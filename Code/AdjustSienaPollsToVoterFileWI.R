##############################################################################
##  Analyze Siena/NYT Wisconsin Data
...
dat <- read_sav("Google Drive/AAPOR.PreElectionPolling/Other Data/2020 General State Level Polls/Siena_Final Datasets/datasetWI.sav")
dat$county_name <- tolower(dat$County)

# Get voter file data

WI_Mode <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/WI/Strata/WI_TargetSmart_PartyStrataVote_2020.csv")
WI_Vote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/WI/Strata/WI_NEAT_PartyStrataVote_2020.csv")

# Get NEAT data for county stratification
wi_ctyvote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/WI/County/WI_NEAT_CountyVote_2020.csv")
wi_ctyvote$county_name <- tolower(wi_ctyvote$county_name)

# Extract information
wi_ctyinfo <- wi_ctyvote[,c("fips_code","county_name","party_stratum","party_stratum_name","geo_stratum","geo_stratum_name")]

Trump.Vote.Strata <- WI_Vote[,c("party_stratum","Trump")]
Biden.Vote.Strata <- WI_Vote[,c("party_stratum","Biden")]
Absentee.Vote.Strata <- WI_Mode[,c("party_stratum","Absentee")]
ElectionDay.Vote.Strata <- WI_Mode[,c("party_stratum","Election.Day")] 
Early.Vote.Strata <- WI_Mode[,c("party_stratum","Early")]
TotalVote <- sum(WI_Vote$Trump+WI_Vote$Biden)
TotalVote2 <- sum(WI_Mode$Absentee+WI_Mode$Election.Day + WI_Mode$Early)
...
Siena_WI <- merge(dat,wi_ctyinfo,by="county_name")
...
Siena_WI$EDVote <- 0
Siena_WI$EDVote[Siena_WI$QMETH2==1] <- 1
Siena_WI$EarlyVote <- 0
Siena_WI$EarlyVote[Siena_WI$QEVMETH==1] <- 1
Siena_WI$EarlyVote[Siena_WI$QMETH2==2] <- 1
Siena_WI$AbsenteeVote <- 0
Siena_WI$AbsenteeVote[Siena_WI$QMETH2==3] <- 1
Siena_WI$AbsenteeVote[Siena_WI$QEVMETH==2] <- 1

for(i in 1:5){
  datsub <- subset(Siena_WI,party_stratum==i)
...
for(i in 1:5){
  Siena_WI$ModeWgt[Siena_WI$party_stratum==i & Siena_WI$EDVote==1] <- EDWgt[i]
  Siena_WI$ModeWgt[Siena_WI$party_stratum==i & Siena_WI$AbsenteeVote==1] <- AbsWgt[i]
  Siena_WI$ModeWgt[Siena_WI$party_stratum==i & Siena_WI$EarlyVote==1] <- EarlyWgt[i]
}

# How correlated are these?  
cor(Siena_WI$final_weight,Siena_WI$ModeWgt,use="pairwise.complete")
...
Siena_WI$WeightModeWgt <- Siena_WI$ModeWgt*Siena_WI$final_weight
...
for(i in 1:5){
  Siena_WI$VoteWgt[Siena_WI$party_stratum==i & Siena_WI$QPRES==1] <- BidenWgt[i]
  Siena_WI$VoteWgt[Siena_WI$party_stratum==i & Siena_WI$QPRES==2] <- TrumpWgt[i]
}

# How correlated are these?  
cor(Siena_WI$final_weight,Siena_WI$VoteWgt,use="pairwise.complete")

Siena_WI$WeightVoteModeWgt <- Siena_WI$VoteWgt*Siena_WI$WeightModeWgt
...
TrumpStrata.WI <- Poll3.Trump.Vote.Strata - Poll.Trump.Vote.Strata
BidenStrata.WI <- Poll3.Biden.Vote.Strata - Poll.Biden.Vote.Strata
...
Siena_WI$QRACE[Siena_WI$QRACE==4] <- 5
Siena_WI$QRACE[Siena_WI$QRACE==6] <- 5
Siena_WI$QRACE[Siena_WI$QRACE==9] <- NA
Siena_WI$GENDER[Siena_WI$GENDER==3] <- NA

Siena_WI$QVOTE16[Siena_WI$QVOTE16==5] <- 8
Siena_WI$QVOTE16[Siena_WI$QVOTE16==7] <- 6

png(file="Google Drive/AAPOR.PreElectionPolling/Writing/Figures/WIAdjust_Siena.png",width=600, height=900)
...
plot(seq(1,ncompare),seq(1,ncompare),xlim=c(a,b),type="n",axes=FALSE,ylab="",
     ...
     main="WI: Effect of Reweighting by Vote and Mode by County Strata")
axis(1)
...
datsub <- na.omit(as.data.frame(cbind(Siena_WI$QRACE,Siena_WI$final_weight,Siena_WI$WeightVoteModeWgt)))
...
# Education
datsub <- na.omit(as.data.frame(cbind(Siena_WI$QEDUC4,Siena_WI$final_weight,Siena_WI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_WI$QEDUC4)
...
# Gender
datsub <- na.omit(as.data.frame(cbind(Siena_WI$GENDER,Siena_WI$final_weight,Siena_WI$WeightVoteModeWgt)))
...
# Age
datsub <- na.omit(as.data.frame(cbind(Siena_WI$AGE4,Siena_WI$final_weight,Siena_WI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_WI$AGE4)[1:4]
...
datsub <- na.omit(as.data.frame(cbind(Siena_WI$QVOTE16,Siena_WI$final_weight,Siena_WI$WeightVoteModeWgt)))
...
# PARTY
datsub <- na.omit(as.data.frame(cbind(Siena_WI$QPARTYID,Siena_WI$final_weight,Siena_WI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_WI$QPARTYID)
...
ChangeWI <- table(Vote2106=Siena_WI$QVOTE16,Vote2020=Siena_WI$QPRES)

# New Voters
sum(as.logical(Siena_WI$QVOTE16==8)*Siena_WI$final_weight)/sum(Siena_WI$final_weight)
sum(as.logical(Siena_WI$QVOTE16==8 & Siena_WI$QPRES==1)*Siena_WI$final_weight)/sum(as.logical(Siena_WI$QVOTE16==8)*Siena_WI$final_weight)
sum(as.logical(Siena_WI$QVOTE16==8 & Siena_WI$QPRES==2)*Siena_WI$final_weight)/sum(as.logical(Siena_WI$QVOTE16==8)*Siena_WI$final_weight)
sum(as.logical(Siena_WI$QVOTE16==8 & Siena_WI$QPRES==9)*Siena_WI$final_weight)/sum(as.logical(Siena_WI$QVOTE16==8)*Siena_WI$final_weight)
