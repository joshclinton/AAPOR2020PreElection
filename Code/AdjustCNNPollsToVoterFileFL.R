...
CNN_FL <- read_spss("Google Drive/AAPOR.PreElectionPolling/Other Data/2020 General State Level Polls/CNN_Final Datasets/CNN_FL.sav")
CNN_FL$fips_code <- CNN_FL$merge_fips
...
CNN_FL <- merge(CNN_FL,fl_ctyinfo,by="fips_code")
#CNN_FL$party_stratum <- CNN_FL$party_stratum.x
nobs2 <- nrow(CNN_FL)
...
CNN_FL$EDVote <- ifelse(CNN_FL$votemode==1,1,0)
CNN_FL$EarlyVote <- ifelse(CNN_FL$votemode==3,1,0)
CNN_FL$Absentee <- ifelse(CNN_FL$votemode==2,1,0)

for(i in 1:5){
  datsub <- subset(CNN_FL,party_stratum==i)
...
for(i in 1:5){
  CNN_FL$ModeWgt[CNN_FL$party_stratum==i & CNN_FL$EDVote==1] <- EDWgt[i]
  CNN_FL$ModeWgt[CNN_FL$party_stratum==i & CNN_FL$EarlyVote==1] <- EarlyWgt[i]
  CNN_FL$ModeWgt[CNN_FL$party_stratum==i & CNN_FL$Absentee==1] <- AbsWgt[i]
}

# How correlated are these?  
cor(CNN_FL$weight,CNN_FL$ModeWgt,use="pairwise.complete")
...
CNN_FL$WeightModeWgt <- CNN_FL$ModeWgt*CNN_FL$weight
...
for(i in 1:5){
  CNN_FL$VoteWgt[CNN_FL$party_stratum==i & CNN_FL$presvote==1] <- BidenWgt[i]
  CNN_FL$VoteWgt[CNN_FL$party_stratum==i & CNN_FL$presvote==2] <- TrumpWgt[i]
}

# How correlated are these?  
cor(CNN_FL$weight,CNN_FL$VoteWgt,use="pairwise.complete")

CNN_FL$WeightVoteModeWgt <- CNN_FL$VoteWgt*CNN_FL$WeightModeWgt
...
png(file="Google Drive/AAPOR.PreElectionPolling/Writing/Figures/CNNFLAdjust.png",width=600, height=900)
...
# White/Non-White
datsub <- na.omit(as.data.frame(cbind(CNN_FL$white,CNN_FL$weight,CNN_FL$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_FL$white)
points(as.numeric(tb),seq(1,3),pch=18)
text(rep(a,times=length(labt)),seq(1,3),labt,pos=4)
abline(h=3.5)

# Education
datsub <- na.omit(as.data.frame(cbind(CNN_FL$educ,CNN_FL$weight,CNN_FL$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_FL$educ)
points(as.numeric(tb),seq(4,8),pch=18)
text(rep(a,times=length(labt)),seq(4,8),labt,pos=4)
abline(h=8.5)

# income
datsub <- na.omit(as.data.frame(cbind(CNN_FL$income3,CNN_FL$weight,CNN_FL$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_FL$income3)
points(as.numeric(tb),seq(9,12),pch=18)
text(rep(a,times=length(labt)),seq(9,12),labt,pos=4)
abline(h=12.5)

# Gender
datsub <- na.omit(as.data.frame(cbind(CNN_FL$sex,CNN_FL$weight,CNN_FL$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_FL$sex)
points(as.numeric(tb),seq(13,14),pch=18)
text(rep(a,times=length(labt)),seq(13,14),labt,pos=4)
abline(h=14.5)

# Age
datsub <- na.omit(as.data.frame(cbind(CNN_FL$age,CNN_FL$weight,CNN_FL$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_FL$age)
points(as.numeric(tb),seq(15,19),pch=18)
text(rep(a,times=length(labt)),seq(15,19),labt,pos=4)
abline(h=19.5)

# ideology
datsub <- na.omit(as.data.frame(cbind(CNN_FL$ideology,CNN_FL$weight,CNN_FL$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_FL$ideology)
points(as.numeric(tb),seq(20,23),pch=18)
text(rep(a,times=length(labt)),seq(20,23),labt,pos=4)
abline(h=23.5)

# PARTY5
datsub <- na.omit(as.data.frame(cbind(CNN_FL$party,CNN_FL$weight,CNN_FL$WeightVoteModeWgt)))
