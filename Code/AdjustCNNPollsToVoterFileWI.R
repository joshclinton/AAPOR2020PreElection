...
CNN_WI <- read_spss("Google Drive/AAPOR.PreElectionPolling/Other Data/2020 General State Level Polls/CNN_Final Datasets/CNN_WI.sav")
CNN_WI$fips_code <- CNN_WI$merge_fips

# Get voter file data

WI_Mode <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/WI_TargetSmart_PartyStrataVote_2020.csv")
WI_Vote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/WI_NEAT_PartyStrataVote_2020.csv")

# Get NEAT data for county stratification
wi_ctyvote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/WI_NEAT_CountyVote_2020.csv")
...
CNN_WI <- merge(CNN_WI,wi_ctyinfo,by="fips_code")
#CNN_WI$party_stratum <- CNN_WI$party_stratum.x
nobs2 <- nrow(CNN_WI)
...
CNN_WI$EDVote <- ifelse(CNN_WI$votemode==1,1,0)
CNN_WI$EarlyVote <- ifelse(CNN_WI$votemode==3,1,0)
CNN_WI$Absentee <- ifelse(CNN_WI$votemode==2,1,0)

for(i in 1:5){
  datsub <- subset(CNN_WI,party_stratum==i)
...
for(i in 1:5){
  CNN_WI$ModeWgt[CNN_WI$party_stratum==i & CNN_WI$EDVote==1] <- EDWgt[i]
  CNN_WI$ModeWgt[CNN_WI$party_stratum==i & CNN_WI$EarlyVote==1] <- EarlyWgt[i]
  CNN_WI$ModeWgt[CNN_WI$party_stratum==i & CNN_WI$Absentee==1] <- AbsWgt[i]
}

# How correlated are these?  
cor(CNN_WI$weight,CNN_WI$ModeWgt,use="pairwise.complete")
...
CNN_WI$WeightModeWgt <- CNN_WI$ModeWgt*CNN_WI$weight
...
for(i in 1:5){
  CNN_WI$VoteWgt[CNN_WI$party_stratum==i & CNN_WI$presvote==1] <- BidenWgt[i]
  CNN_WI$VoteWgt[CNN_WI$party_stratum==i & CNN_WI$presvote==2] <- TrumpWgt[i]
}

# How correlated are these?  
cor(CNN_WI$weight,CNN_WI$VoteWgt,use="pairwise.complete")

CNN_WI$WeightVoteModeWgt <- CNN_WI$VoteWgt*CNN_WI$WeightModeWgt
...
png(file="Google Drive/AAPOR.PreElectionPolling/Writing/Figures/CNNWIAdjust.png",width=600, height=900)
a <- -10
b <- 5
ncompare <- 28
plot(seq(1,ncompare),seq(1,ncompare),xlim=c(a,b),type="n",axes=FALSE,ylab="",
     xlab="Change in % After Weighting",
...
# White/Non-White
datsub <- na.omit(as.data.frame(cbind(CNN_WI$white,CNN_WI$weight,CNN_WI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_WI$white)
points(as.numeric(tb),seq(1,3),pch=18)
text(rep(a,times=length(labt)),seq(1,3),labt,pos=4)
abline(h=3.5)

# Education
datsub <- na.omit(as.data.frame(cbind(CNN_WI$educ,CNN_WI$weight,CNN_WI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_WI$educ)
points(as.numeric(tb),seq(4,8),pch=18)
text(rep(a,times=length(labt)),seq(4,8),labt,pos=4)
abline(h=8.5)

# income
datsub <- na.omit(as.data.frame(cbind(CNN_WI$income3,CNN_WI$weight,CNN_WI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_WI$income3)
points(as.numeric(tb),seq(9,12),pch=18)
text(rep(a,times=length(labt)),seq(9,12),labt,pos=4)
abline(h=12.5)

# Gender
datsub <- na.omit(as.data.frame(cbind(CNN_WI$sex,CNN_WI$weight,CNN_WI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_WI$sex)
points(as.numeric(tb),seq(13,14),pch=18)
text(rep(a,times=length(labt)),seq(13,14),labt,pos=4)
abline(h=14.5)

# Age
datsub <- na.omit(as.data.frame(cbind(CNN_WI$age,CNN_WI$weight,CNN_WI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_WI$age)
points(as.numeric(tb),seq(15,19),pch=18)
text(rep(a,times=length(labt)),seq(15,19),labt,pos=4)
abline(h=19.5)

# ideology
datsub <- na.omit(as.data.frame(cbind(CNN_WI$ideology,CNN_WI$weight,CNN_WI$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(CNN_WI$ideology)
points(as.numeric(tb),seq(20,23),pch=18)
text(rep(a,times=length(labt)),seq(20,23),labt,pos=4)
abline(h=23.5)

# PARTY5
datsub <- na.omit(as.data.frame(cbind(CNN_WI$party,CNN_WI$weight,CNN_WI$WeightVoteModeWgt)))
