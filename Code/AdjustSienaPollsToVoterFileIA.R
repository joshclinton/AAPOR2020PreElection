...
dat <- read_sav("Google Drive/AAPOR.PreElectionPolling/Other Data/2020 General State Level Polls/Siena_Final Datasets/datasetIA.sav")
dat$county_name <- tolower(dat$County)

# Get voter file data

IA_Mode <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/IA/Strata/IA_TargetSmart_PartyStrataVote_2020.csv")
IA_Vote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/IA/Strata/IA_NEAT_PartyStrataVote_2020.csv")

# Get NEAT data for county stratification
ia_ctyvote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/IA/County/IA_NEAT_CountyVote_2020.csv")
ia_ctyvote$county_name <- tolower(ia_ctyvote$county_name)

# Extract information
ia_ctyinfo <- ia_ctyvote[,c("fips_code","county_name","party_stratum","party_stratum_name","geo_stratum","geo_stratum_name")]

Trump.Vote.Strata <- IA_Vote[,c("party_stratum","Trump")]
Biden.Vote.Strata <- IA_Vote[,c("party_stratum","Biden")]
Absentee.Vote.Strata <- IA_Mode[,c("party_stratum","Absentee")]
ElectionDay.Vote.Strata <- IA_Mode[,c("party_stratum","Election.Day")] 
Early.Vote.Strata <- IA_Mode[,c("party_stratum","Early")]
TotalVote <- sum(IA_Vote$Trump+IA_Vote$Biden)
TotalVote2 <- sum(IA_Mode$Absentee+IA_Mode$Election.Day + IA_Mode$Early)
...
Siena_IA <- merge(dat,ia_ctyinfo,by="county_name")
...
Siena_IA$EDVote <- 0
Siena_IA$EDVote[Siena_IA$QMETH2==1] <- 1
Siena_IA$EarlyVote <- 0
Siena_IA$EarlyVote[Siena_IA$QEVMETH==1] <- 1
Siena_IA$EarlyVote[Siena_IA$QMETH2==2] <- 1
Siena_IA$AbsenteeVote <- 0
Siena_IA$AbsenteeVote[Siena_IA$QMETH2==3] <- 1
Siena_IA$AbsenteeVote[Siena_IA$QEVMETH==2] <- 1

for(i in 1:5){
  datsub <- subset(Siena_IA,party_stratum==i)
...
for(i in 1:5){
  Siena_IA$ModeWgt[Siena_IA$party_stratum==i & Siena_IA$EDVote==1] <- EDWgt[i]
  Siena_IA$ModeWgt[Siena_IA$party_stratum==i & Siena_IA$AbsenteeVote==1] <- AbsWgt[i]
  Siena_IA$ModeWgt[Siena_IA$party_stratum==i & Siena_IA$EarlyVote==1] <- EarlyWgt[i]
}

# How correlated are these?  
cor(Siena_IA$final_weight,Siena_IA$ModeWgt,use="pairwise.complete")
...
Siena_IA$WeightModeWgt <- Siena_IA$ModeWgt*Siena_IA$final_weight
...
for(i in 1:5){
  Siena_IA$VoteWgt[Siena_IA$party_stratum==i & Siena_IA$QPRES==1] <- BidenWgt[i]
  Siena_IA$VoteWgt[Siena_IA$party_stratum==i & Siena_IA$QPRES==2] <- TrumpWgt[i]
}

# How correlated are these?  
cor(Siena_IA$final_weight,Siena_IA$VoteWgt,use="pairwise.complete")

Siena_IA$WeightVoteModeWgt <- Siena_IA$VoteWgt*Siena_IA$WeightModeWgt
...
TrumpStrata.IA <- Poll3.Trump.Vote.Strata - Poll.Trump.Vote.Strata
BidenStrata.IA <- Poll3.Biden.Vote.Strata - Poll.Biden.Vote.Strata
...
Siena_IA$QRACE[Siena_IA$QRACE==4] <- 5
Siena_IA$QRACE[Siena_IA$QRACE==6] <- 5
Siena_IA$QRACE[Siena_IA$QRACE==9] <- NA
Siena_IA$GENDER[Siena_IA$GENDER==3] <- NA

Siena_IA$QVOTE16[Siena_IA$QVOTE16==5] <- 8
Siena_IA$QVOTE16[Siena_IA$QVOTE16==7] <- 6

png(file="Google Drive/AAPOR.PreElectionPolling/Writing/Figures/IAAdjust_Siena.png",width=600, height=900)
...
plot(seq(1,ncompare),seq(1,ncompare),xlim=c(a,b),type="n",axes=FALSE,ylab="",
     ...
     main="IA: Effect of Reweighting by Vote and Mode by County Strata")
axis(1)
...
datsub <- na.omit(as.data.frame(cbind(Siena_IA$QRACE,Siena_IA$final_weight,Siena_IA$WeightVoteModeWgt)))
...
# Education
datsub <- na.omit(as.data.frame(cbind(Siena_IA$QEDUC4,Siena_IA$final_weight,Siena_IA$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_IA$QEDUC4)
...
# Gender
datsub <- na.omit(as.data.frame(cbind(Siena_IA$GENDER,Siena_IA$final_weight,Siena_IA$WeightVoteModeWgt)))
...
# Age
datsub <- na.omit(as.data.frame(cbind(Siena_IA$AGE4,Siena_IA$final_weight,Siena_IA$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_IA$AGE4)[1:4]
...
datsub <- na.omit(as.data.frame(cbind(Siena_IA$QVOTE16,Siena_IA$final_weight,Siena_IA$WeightVoteModeWgt)))
...
# PARTY
datsub <- na.omit(as.data.frame(cbind(Siena_IA$QPARTYID,Siena_IA$final_weight,Siena_IA$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_IA$QPARTYID)
...
ChangeIA <- table(Vote2106=Siena_IA$QVOTE16,Vote2020=Siena_IA$QPRES)

# New Voters
sum(as.logical(Siena_IA$QVOTE16==8)*Siena_IA$final_weight)/sum(Siena_IA$final_weight)
sum(as.logical(Siena_IA$QVOTE16==8 & Siena_IA$QPRES==1)*Siena_IA$final_weight)/sum(as.logical(Siena_IA$QVOTE16==8)*Siena_IA$final_weight)
sum(as.logical(Siena_IA$QVOTE16==8 & Siena_IA$QPRES==2)*Siena_IA$final_weight)/sum(as.logical(Siena_IA$QVOTE16==8)*Siena_IA$final_weight)
sum(as.logical(Siena_IA$QVOTE16==8 & Siena_IA$QPRES==9)*Siena_IA$final_weight)/sum(as.logical(Siena_IA$QVOTE16==8)*Siena_IA$final_weight)
sum(as.logical(Siena_IA$QVOTE16==8 & Siena_IA$QPRES==9 & Siena_IA$QPRESLN==1)*Siena_IA$final_weight)/sum(as.logical(Siena_IA$QVOTE16==8 & Siena_IA$QPRES==9)*Siena_IA$final_weight)
sum(as.logical(Siena_IA$QVOTE16==8 & Siena_IA$QPRES==9 & Siena_IA$QPRESLN==2)*Siena_IA$final_weight)/sum(as.logical(Siena_IA$QVOTE16==8 & Siena_IA$QPRES==9)*Siena_IA$final_weight)
