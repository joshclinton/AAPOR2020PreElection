##############################################################################
##  Analyze Siena/NYT North Carolina Data
...
dat <- read_sav("Google Drive/AAPOR.PreElectionPolling/Other Data/2020 General State Level Polls/Siena_Final Datasets/datasetNC.sav")
dat$county_name <- tolower(dat$County)

# Get voter file data

NC_Mode <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/NC_TargetSmart_PartyStrataVote_2020.csv")
NC_Vote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/NC_NEAT_PartyStrataVote_2020.csv")

# Get NEAT data for county stratification
nc_ctyvote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/NC_NEAT_CountyVote_2020.csv")
nc_ctyvote$county_name <- tolower(nc_ctyvote$county_name)

# Extract information
nc_ctyinfo <- nc_ctyvote[,c("fips_code","county_name","party_stratum","party_stratum_name","geo_stratum","geo_stratum_name")]

Trump.Vote.Strata <- NC_Vote[,c("party_stratum","Trump")]
Biden.Vote.Strata <- NC_Vote[,c("party_stratum","Biden")]
Absentee.Vote.Strata <- NC_Mode[,c("party_stratum","Absentee")]
ElectionDay.Vote.Strata <- NC_Mode[,c("party_stratum","Election.Day")]
Early.Vote.Strata <- NC_Mode[,c("party_stratum","Early")]
TotalVote <- sum(NC_Vote$Trump+NC_Vote$Biden)
TotalVote2 <- sum(NC_Mode$Absentee+NC_Mode$Election.Day + NC_Mode$Early)
...
Siena_NC <- merge(dat,nc_ctyinfo,by="county_name")
...
Siena_NC$EDVote <- 0
Siena_NC$EDVote[Siena_NC$QMETH2==1] <- 1
Siena_NC$EarlyVote <- 0
Siena_NC$EarlyVote[Siena_NC$QEVMETH==1] <- 1
Siena_NC$EarlyVote[Siena_NC$QMETH2==2] <- 1
Siena_NC$AbsenteeVote <- 0
Siena_NC$AbsenteeVote[Siena_NC$QMETH2==3] <- 1
Siena_NC$AbsenteeVote[Siena_NC$QEVMETH==2] <- 1

for(i in 1:5){
  datsub <- subset(Siena_NC,party_stratum==i)
...
for(i in 1:5){
  Siena_NC$ModeWgt[Siena_NC$party_stratum==i & Siena_NC$EDVote==1] <- EDWgt[i]
  Siena_NC$ModeWgt[Siena_NC$party_stratum==i & Siena_NC$AbsenteeVote==1] <- AbsWgt[i]
  Siena_NC$ModeWgt[Siena_NC$party_stratum==i & Siena_NC$EarlyVote==1] <- EarlyWgt[i]
}

# How correlated are these?  
cor(Siena_NC$final_weight,Siena_NC$ModeWgt,use="pairwise.complete")
...
Siena_NC$WeightModeWgt <- Siena_NC$ModeWgt*Siena_NC$final_weight
...
for(i in 1:5){
  Siena_NC$VoteWgt[Siena_NC$party_stratum==i & Siena_NC$QPRES==1] <- BidenWgt[i]
  Siena_NC$VoteWgt[Siena_NC$party_stratum==i & Siena_NC$QPRES==2] <- TrumpWgt[i]
}

# How correlated are these?  
cor(Siena_NC$final_weight,Siena_NC$VoteWgt,use="pairwise.complete")

Siena_NC$WeightVoteModeWgt <- Siena_NC$VoteWgt*Siena_NC$WeightModeWgt
...
TrumpStrata.NC <- Poll3.Trump.Vote.Strata - Poll.Trump.Vote.Strata
BidenStrata.NC <- Poll3.Biden.Vote.Strata - Poll.Biden.Vote.Strata

############################################################
#     NOW LOOK AT IMNCCT ON DEMOGRAPHICS
...
Siena_NC$QRACE[Siena_NC$QRACE==4] <- 5
Siena_NC$QRACE[Siena_NC$QRACE==6] <- 5
Siena_NC$QRACE[Siena_NC$QRACE==9] <- NA
Siena_NC$GENDER[Siena_NC$GENDER==3] <- NA

Siena_NC$QVOTE16[Siena_NC$QVOTE16==5] <- 8
Siena_NC$QVOTE16[Siena_NC$QVOTE16==7] <- 6

png(file="Google Drive/AAPOR.PreElectionPolling/Writing/Figures/NCAdjust_Siena.png",width=600, height=900)
...
plot(seq(1,ncompare),seq(1,ncompare),xlim=c(a,b),type="n",axes=FALSE,ylab="",
     ...
     main="NC: Effect of Reweighting by Vote and Mode by County Strata")
axis(1)
...
datsub <- na.omit(as.data.frame(cbind(Siena_NC$QRACE,Siena_NC$final_weight,Siena_NC$WeightVoteModeWgt)))
...
# Education
datsub <- na.omit(as.data.frame(cbind(Siena_NC$QEDUC4,Siena_NC$final_weight,Siena_NC$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_NC$QEDUC4)
...
# Gender
datsub <- na.omit(as.data.frame(cbind(Siena_NC$GENDER,Siena_NC$final_weight,Siena_NC$WeightVoteModeWgt)))
...
# Age
datsub <- na.omit(as.data.frame(cbind(Siena_NC$AGE4,Siena_NC$final_weight,Siena_NC$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_NC$AGE4)[1:4]
...
datsub <- na.omit(as.data.frame(cbind(Siena_NC$QVOTE16,Siena_NC$final_weight,Siena_NC$WeightVoteModeWgt)))
...
# PARTY
datsub <- na.omit(as.data.frame(cbind(Siena_NC$QPARTYID,Siena_NC$final_weight,Siena_NC$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_NC$QPARTYID)
...
ChangeNC <- table(Vote2106=Siena_NC$QVOTE16,Vote2020=Siena_NC$QPRES)

# New Voters
sum(as.logical(Siena_NC$QVOTE16==8)*Siena_NC$final_weight)/sum(Siena_NC$final_weight)
sum(as.logical(Siena_NC$QVOTE16==8 & Siena_NC$QPRES==1)*Siena_NC$final_weight)/sum(as.logical(Siena_NC$QVOTE16==8)*Siena_NC$final_weight)
sum(as.logical(Siena_NC$QVOTE16==8 & Siena_NC$QPRES==2)*Siena_NC$final_weight)/sum(as.logical(Siena_NC$QVOTE16==8)*Siena_NC$final_weight)
sum(as.logical(Siena_NC$QVOTE16==8 & Siena_NC$QPRES==9)*Siena_NC$final_weight)/sum(as.logical(Siena_NC$QVOTE16==8)*Siena_NC$final_weight)
