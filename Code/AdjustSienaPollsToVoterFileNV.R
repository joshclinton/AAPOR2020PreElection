...
dat <- read_sav("Google Drive/AAPOR.PreElectionPolling/Other Data/2020 General State Level Polls/Siena_Final Datasets/datasetNV.sav")
dat$county_name <- tolower(dat$County)

# Get voter file data

NV_Mode <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/NV/Strata/NV_TargetSmart_PartyStrataVote_2020.csv")
NV_Vote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/NV/Strata/NV_NEAT_PartyStrataVote_2020.csv")

# Get NEAT data for county stratification
nv_ctyvote <- read.csv("Google Drive/AAPOR.PreElectionPolling/Other Data/StateCertifiedVoteTotalsMode/2020 Vote History/NV/County/NV_NEAT_CountyVote_2020.csv")
nv_ctyvote$county_name <- tolower(nv_ctyvote$county_name)

# Extract information
nv_ctyinfo <- nv_ctyvote[,c("fips_code","county_name","party_stratum","party_stratum_name","geo_stratum","geo_stratum_name")]

Trump.Vote.Strata <- NV_Vote[,c("party_stratum","Trump")]
Biden.Vote.Strata <- NV_Vote[,c("party_stratum","Biden")]
Absentee.Vote.Strata <- NV_Mode[,c("party_stratum","Absentee")]
ElectionDay.Vote.Strata <- NV_Mode[,c("party_stratum","Election.Day")] # Also Provisional Questionable
Early.Vote.Strata <- NV_Mode[,c("party_stratum","Early")]
TotalVote <- sum(NV_Vote$Trump+NV_Vote$Biden)
TotalVote2 <- sum(NV_Mode$Absentee+NV_Mode$Election.Day + NV_Mode$Early)
...
Siena_NV <- merge(dat,nv_ctyinfo,by="county_name")
...
Siena_NV$EDVote <- 0
Siena_NV$EDVote[Siena_NV$QMETH2==1] <- 1
Siena_NV$EarlyVote <- 0
Siena_NV$EarlyVote[Siena_NV$QEVMETH==1] <- 1
Siena_NV$EarlyVote[Siena_NV$QMETH2==2] <- 1
Siena_NV$AbsenteeVote <- 0
Siena_NV$AbsenteeVote[Siena_NV$QMETH2==3] <- 1
Siena_NV$AbsenteeVote[Siena_NV$QEVMETH==2] <- 1

for(i in 1:4){
  datsub <- subset(Siena_NV,party_stratum==i)
...
for(i in 1:4){
  Siena_NV$ModeWgt[Siena_NV$party_stratum==i & Siena_NV$EDVote==1] <- EDWgt[i]
  Siena_NV$ModeWgt[Siena_NV$party_stratum==i & Siena_NV$AbsenteeVote==1] <- AbsWgt[i]
  Siena_NV$ModeWgt[Siena_NV$party_stratum==i & Siena_NV$EarlyVote==1] <- EarlyWgt[i]
}

# How correlated are these?  
cor(Siena_NV$final_weight,Siena_NV$ModeWgt,use="pairwise.complete")
...
Siena_NV$WeightModeWgt <- Siena_NV$ModeWgt*Siena_NV$final_weight
...
for(i in 1:4){
  Siena_NV$VoteWgt[Siena_NV$party_stratum==i & Siena_NV$QPRES==1] <- BidenWgt[i]
  Siena_NV$VoteWgt[Siena_NV$party_stratum==i & Siena_NV$QPRES==2] <- TrumpWgt[i]
}

# How correlated are these?  
cor(Siena_NV$final_weight,Siena_NV$VoteWgt,use="pairwise.complete")

Siena_NV$WeightVoteModeWgt <- Siena_NV$VoteWgt*Siena_NV$WeightModeWgt
...
TrumpStrata.NV <- Poll3.Trump.Vote.Strata - Poll.Trump.Vote.Strata
BidenStrata.NV <- Poll3.Biden.Vote.Strata - Poll.Biden.Vote.Strata
...
Siena_NV$QRACE[Siena_NV$QRACE==4] <- 5
Siena_NV$QRACE[Siena_NV$QRACE==6] <- 5
Siena_NV$QRACE[Siena_NV$QRACE==9] <- NA
Siena_NV$GENDER[Siena_NV$GENDER==3] <- NA

Siena_NV$QVOTE16[Siena_NV$QVOTE16==5] <- 8
Siena_NV$QVOTE16[Siena_NV$QVOTE16==7] <- 6

png(file="Google Drive/AAPOR.PreElectionPolling/Writing/Figures/NVAdjust_Siena.png",width=600, height=900)
...
plot(seq(1,ncompare),seq(1,ncompare),xlim=c(a,b),type="n",axes=FALSE,ylab="",
     ...
     main="NV: Effect of Reweighting by Vote and Mode by County Strata")
axis(1)
...
datsub <- na.omit(as.data.frame(cbind(Siena_NV$QRACE,Siena_NV$final_weight,Siena_NV$WeightVoteModeWgt)))
...
# Education
datsub <- na.omit(as.data.frame(cbind(Siena_NV$QEDUC4,Siena_NV$final_weight,Siena_NV$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_NV$QEDUC4)
...
# Gender
datsub <- na.omit(as.data.frame(cbind(Siena_NV$GENDER,Siena_NV$final_weight,Siena_NV$WeightVoteModeWgt)))
...
# Age
datsub <- na.omit(as.data.frame(cbind(Siena_NV$AGE4,Siena_NV$final_weight,Siena_NV$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_NV$AGE4)[1:4]
...
datsub <- na.omit(as.data.frame(cbind(Siena_NV$QVOTE16,Siena_NV$final_weight,Siena_NV$WeightVoteModeWgt)))
...
# PARTY
datsub <- na.omit(as.data.frame(cbind(Siena_NV$QPARTYID,Siena_NV$final_weight,Siena_NV$WeightVoteModeWgt)))
...
tb <- tb*100
labt <- get_labels(Siena_NV$QPARTYID)
...
ChangeNV <- table(Vote2106=Siena_NV$QVOTE16,Vote2020=Siena_NV$QPRES)

# New Voters
sum(as.logical(Siena_NV$QVOTE16==8)*Siena_NV$final_weight)/sum(Siena_NV$final_weight)
sum(as.logical(Siena_NV$QVOTE16==8 & Siena_NV$QPRES==1)*Siena_NV$final_weight)/sum(as.logical(Siena_NV$QVOTE16==8)*Siena_NV$final_weight)
sum(as.logical(Siena_NV$QVOTE16==8 & Siena_NV$QPRES==2)*Siena_NV$final_weight)/sum(as.logical(Siena_NV$QVOTE16==8)*Siena_NV$final_weight)
sum(as.logical(Siena_NV$QVOTE16==8 & Siena_NV$QPRES==9)*Siena_NV$final_weight)/sum(as.logical(Siena_NV$QVOTE16==8)*Siena_NV$final_weight)
sum(as.logical(Siena_NV$QVOTE16==8 & Siena_NV$QPRES==9 & Siena_NV$QPRESLN==1)*Siena_NV$final_weight)/sum(as.logical(Siena_NV$QVOTE16==8 & Siena_NV$QPRES==9)*Siena_NV$final_weight)
sum(as.logical(Siena_NV$QVOTE16==8 & Siena_NV$QPRES==9 & Siena_NV$QPRESLN==2)*Siena_NV$final_weight)/sum(as.logical(Siena_NV$QVOTE16==8 & Siena_NV$QPRES==9)*Siena_NV$final_weight)
